...
export default function Hero() {
  ...
  const getData = async () => {
    try {
      const q = query(collection(firestore, "uploadProduct"), where("status", "==", "Present"))
  ...
  return (
    <>
      ...
      <div className="container mt-4">
        <div className="row">
          <div className="col d-flex justify-content-center">
            <Link to='/inActive' className='btn btn-light' style={{fontSize:"23px"}}>Go To Check In-Active</Link>
          </div>
        </div>
      </div>
      ...
      <div className="container">
        <div className="row">
          <div className="col">
            <h3 className='text-center my-4 mt-5'>Active Students ({documents.length})</h3>
          </div>
        </div>
      </div>
      ...
      <div className="container ms-5 mt-3" style={{ height: "500px",width:"1100px", position: "absolute" }} >
      {/* 
